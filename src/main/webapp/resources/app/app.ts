...
import ElementModel = model.LearningElementMeta;
	...
	class DNDMechanism {
		...
		private eleme
		public dragStart(evnt:JQueryEventObject) {
			this.elementID = evnt.target.id;
		}

		public drop(evnt:JQueryEventObject) {
			var element: JQuery = jQuery(document.getElementById(this.elementID));
			var letype:string = element.find('.type').html();
			...
			if (letype == 'quiz' || letype == 'class') {
				jQuery.ajax({
					type: 'POST',
					url: '/PseudoLOOP/LOIDE/api/file/prepare/' + this.elementID,
				...
				switch (letype) {
					case "audio":
						jQuery(evnt.target).append('<input class="le-id" type="hidden" value="' + this.elementID + '"><audio class="le-object" src=/PseudoLOOP/LOIDE/api/file/retrieve/' + this.elementID + ' controls />');
						break;
					case "video":
						jQuery(evnt.target).append('<input class="le-id" type="hidden" value="' + this.elementID + '"><video class="le-object" src=/PseudoLOOP/LOIDE/api/file/retrieve/' + this.elementID + ' controls />');
						break;
					case "text":
						this.getTextData(this.elementID, evnt.target);
						break;
					case "image":
						jQuery(evnt.target).append('<input class="le-id" type="hidden" value="' + this.elementID + '"><img class="le-object" src=/PseudoLOOP/LOIDE/api/file/retrieve/' + this.elementID + ' />');
						break;
					case "quiz":
						{
		...
		private getTextData(id:string, target:any) {
			jQuery.get('/PseudoLOOP/LOIDE/api/file/retrieve/' + id, function(data, msg, jqXHR) {
	...
	function initializeDragonLOIDE() {
		jQuery('#pages').tabs();
		jQuery('#pageAddBtn').on('click', function(evnt: JQueryEventObject) {
			evnt.preventDefault();
			var page: number = (jQuery('#pages >ul >li').length + 1);
		...
		$("body").on("click", ".ui-closable-tab", function() {
			...
			$("#pages").tabs("refresh");
			var page: number = jQuery('#pages >ul >li').length;
		...
		jQuery('body').on('click', '.le-obj-del', function(evnt:JQueryEventObject) {
			jQuery(evnt.target).parent().detach();
		});

		var DNDHandler: DNDMechanism = new DNDMechanism();

		jQuery('body').on('dragover', '.slot', function(evnt: JQueryEventObject) {
			evnt.preventDefault();			
		});
		...
		jQuery('#searchForm').on('submit', function(evnt:JQueryEventObject) {
			evnt.preventDefault();
			jQuery.get('/PseudoLOOP/LOIDE/api/le/query/' + jQuery('#searchBox').val(),
					   function(data, status, jqXHR) {
						   var results:ElementModel[] = data;
		...
		jQuery('#leForm').on('submit', function(evnt: JQueryEventObject) { 
			evnt.preventDefault();
			...
			if (complete) {
				var data: FormData = new FormData(<any>evnt.target);

				jQuery.ajax({
					type: 'POST',
					url: '/PseudoLOOP/LOIDE/api/file/upload',
					processData: false,
					contentType: false,
					data: data,
					success: function(data, status, jqXHR) {
		...
		jQuery('#quizForm').on('submit', function(evnt: JQueryEventObject) {
			evnt.preventDefault();
		...
		jQuery('#loForm').on('submit', function(evnt: JQueryEventObject) {
			evnt.preventDefault();
			...
			if (complete) {
				var data: any = new FormData(<any>evnt.target);
				...
				jQuery.ajax({
					type: 'POST',
					url: '/PseudoLOOP/LOIDE/api/file/upload/lo',
					processData: false,
					contentType: false,
					data: data,
					success: function(data, status, jqXHR) {
	...
	function toElement(data:ElementModel) : string {
		var elem: string;
